...
namespace AlteredCarbon
{
    ...
    public class Window_SleeveCustomization : Window
    {
        ...
        private int sleevePsychicSensitivityIndex = 2;
        ...
        public static Dictionary<TraitDef, List<int>> traitsTimeCost = new Dictionary<TraitDef, List<int>>
        {
            { AC_DefOf.Beauty, new List<int>
                {
                    0,
                    GenDate.TicksPerDay * 1,
                    GenDate.TicksPerDay * 2,
                    GenDate.TicksPerDay * 4,
                    GenDate.TicksPerDay * 5
                }
            },

            { AC_DefOf.PsychicSensitivity, new List<int>
                {
                    0,
                    GenDate.TicksPerDay * 1,
                    GenDate.TicksPerDay * 2,
        ...
        private static readonly Dictionary<TraitDef, List<int>> traitDegrees = new Dictionary<TraitDef, List<int>>
        {
            { AC_DefOf.Beauty, new List<int> { -2, -1, 0, 1, 2 } },
            { AC_DefOf.PsychicSensitivity, new List<int> { -2, -1, 0, 1, 2 } },
        };
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            DoSelectionButtons(ref firstColumnPos, "AC.SleeveBeauty".Translate(), ref sleeveBeautyIndex,
                (int x) => GetTraitLabel(AC_DefOf.Beauty, x), traitDegrees[AC_DefOf.Beauty], delegate (int x)
                {
                    sleeveBeautyIndex = traitDegrees[AC_DefOf.Beauty].IndexOf(x);
                    ApplyTrait(AC_DefOf.Beauty, sleeveBeautyIndex);
                    UpdateGrowCost();
                }, floatMenu: true);

            DoSelectionButtons(ref firstColumnPos, "AC.PsychicSensitivity".Translate(), ref sleevePsychicSensitivityIndex,
                (int x) => GetTraitLabel(AC_DefOf.PsychicSensitivity, x), traitDegrees[AC_DefOf.PsychicSensitivity], delegate (int x)
                {
                    sleevePsychicSensitivityIndex = traitDegrees[AC_DefOf.PsychicSensitivity].IndexOf(x);
                    ApplyTrait(AC_DefOf.PsychicSensitivity, sleevePsychicSensitivityIndex);
                    UpdateGrowCost();
                }, floatMenu: true);
        ...
        public void UpdateGrowCost()
        {
            ...
            ticksToGrow += traitsTimeCost[AC_DefOf.Beauty][sleeveBeautyIndex];
            ticksToGrow += traitsTimeCost[AC_DefOf.PsychicSensitivity][sleevePsychicSensitivityIndex];
        ...
        public void ApplyTrait(TraitDef traitDef, int traitIndex)
        {
            Trait trait = curSleeve.story.traits.GetTrait(traitDef);
            if (trait != null)
            {
                curSleeve.story.traits.RemoveTrait(trait);
            }
            if (traitDegrees[traitDef][traitIndex] != 0)
            {
                curSleeve.story.traits.GainTrait(new Trait(traitDef, traitDegrees[traitDef][traitIndex]));
            }
        }

        public string GetTraitLabel(TraitDef traitDef, int index)
        {
            if (index != 0)
            {
                Trait trait = new Trait(traitDef, index);
                return trait.LabelCap;
            }
            else
            {
        ...
        private void RecheckBodyOptions()
        {
            ...
            foreach (var traitDef in traitDegrees.Keys)
            {
                var trait = curSleeve.story.traits.GetTrait(traitDef);
                if (trait != null)
                {
                    if (traitDef == AC_DefOf.Beauty)
                    {
                        sleeveBeautyIndex = traitDegrees[traitDef].IndexOf(trait.degree);
                    }
                    else if (traitDef == AC_DefOf.PsychicSensitivity)
                    {
                        sleevePsychicSensitivityIndex = traitDegrees[traitDef].IndexOf(trait.degree);
                    }
                }
                else
                {
                    ...
                    {
                        sleevePsychicSensitivityIndex = 2;
                    }
                }
            }
        }
