...
namespace AlteredCarbon
{
    ...
    public static class AC_Utils
    {
        ...
        public static Building_NeuralMatrix GetConnectedMatrix(this ThingWithComps thing, bool checkForPower = true)
        {
            if (thing is Building_NeuralMatrix matrix)
            {
                return matrix;
            }
            var compFacility = thing.TryGetComp<CompAffectedByFacilities>();
            return compFacility.LinkedFacilitiesListForReading.OfType<Building_NeuralMatrix>()
                .FirstOrDefault(x => checkForPower is false || x.Powered);
        }
        ...
        static AC_Utils()
        {
            ...
            {
                var compIndex = storyteller.comps.FindIndex(x => x is StorytellerCompProperties_SingleOnceFixed);
                if (compIndex >= 0)
                {
        ...
        public static Pawn ClonePawn(Pawn source, bool copyTattoos = false)
        {
            ...
            {
                clone.creepjoiner = new Pawn_CreepJoinerTracker(clone)
                {
            ...
            foreach (var trait in source.story.traits.allTraits)
            {
                ...
                {
                    if (clone.story.traits.HasTrait(trait.def) is false)
                    {
        ...
        {
            var comp = source.GetComp<VEF.Abilities.CompAbilities>();
            if (comp != null && comp.LearnedAbilities != null)
            {
                var veAbilities = comp.LearnedAbilities.Select(x => x.def).ToList();
                var otherComp = dest.GetComp<VEF.Abilities.CompAbilities>();
                if (otherComp != null)
                {
        ...
        public static string GetFullName(this Pawn pawn)
        {
            ...
            bool original = pawn.IsOriginal();
            bool copy = pawn.IsCopy();
            return pawn.Name?.ToStringFull + " (copy: " + copy.ToString() + ") (original: " + original.ToString()
                + ") - " + pawn.thingIDNumber;
        }
        ...
        public static void LoadPresets()
        {
            ...
            if (presets != null)
            {
                ...
                {
                    preset.Value.sleeve.thingIDNumber = Find.UniqueIDsManager.GetNextThingID();
                }
            }
        }
        ...
        public static Dictionary<Pawn, ConnectStatus> GetAllConnectablePawnsFor(INeedlecastable needlecastable)
        {
            ...
            foreach (var otherPawn in PawnsFinder.AllMapsCaravansAndTravellingTransporters_Alive)
            {
                if (otherPawn.HasRemoteStack(out var remoteStack) && remoteStack.Needlecasted is false)
                {
                    ...
                    if (connectStatus != ConnectStatus.ConnectionDisrupted)
                    {
        ...
        public static string GetLabel(this ConnectStatus connectStatus)
        {
            ...
            else if (connectStatus == ConnectStatus.LowConscioussness)
            {
                return "AC.LowConscioussness".Translate();
            }
        ...
        {
            var entropy = pawn.psychicEntropy?.currentEntropy ?? 0;
            var psyfocus = pawn.psychicEntropy?.currentPsyfocus ?? 0;
        ...
        public static bool IsMyOriginal(this Pawn copy, Pawn pawn)
        {
            if (pawn is null) return false;

            if (copy.HasNeuralStack(out var hediff))
            {
                var stackGroupData = hediff.NeuralData.StackGroupData;
            ...
            else
            {
                foreach (var stackGroup in AlteredCarbonManager.Instance.stacksRelationships)
                {
                    if (stackGroup.Value.originalPawn == pawn && stackGroup.Value.copiedPawns.Contains(copy))
                    {
                        return true;
                    }
                }
        ...
        public static bool IsDummyPawn(this Pawn pawn)
        {
            return NeuralData.dummyPawns.KeyExists(pawn);
        }
        ...
        public static void MakeEmptySleeve(this Pawn pawn)
        {
            ...
            AlteredCarbonManager.Instance.emptySleeves.Add(pawn);
            var catatonic = pawn.GetHediff(HediffDefOf.CatatonicBreakdown);
            if (catatonic != null)
            {
