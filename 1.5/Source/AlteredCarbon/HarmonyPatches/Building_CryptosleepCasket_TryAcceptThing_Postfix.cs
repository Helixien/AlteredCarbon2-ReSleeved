...
namespace AlteredCarbon
{
    ...
    public static class Building_CryptosleepCasket_TryAcceptThing_Postfix
    {
        ...
        {
            if (__result && thing is Pawn pawn && pawn.Faction == Faction.OfPlayer)
            {
                if (pawn.HasNeuralStack(out var neural))
                {
                ...
                else if (pawn.HasRemoteStack(out var remoteStack) && remoteStack.Needlecasted)
                {
                    remoteStack.EndNeedlecasting(ConnectStatus.ConnectionDisrupted);
                }
            }
        }
    }
