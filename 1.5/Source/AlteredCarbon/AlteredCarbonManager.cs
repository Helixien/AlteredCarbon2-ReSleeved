...
namespace AlteredCarbon
{
    ...
    public class AlteredCarbonManager : GameComponent
    {
        ...
        public void RegisterExtractedStack(Pawn pawn, NeuralStack stack)
        {
        ...
        public List<(int frame, Action action)> actionsToDo = new();
        public override void GameComponentUpdate()
        {
            base.GameComponentUpdate();
            for (int i = actionsToDo.Count - 1; i >= 0; i--)
            {
                if (Time.frameCount >= actionsToDo[i].frame)
                {
                    actionsToDo[i].action();
