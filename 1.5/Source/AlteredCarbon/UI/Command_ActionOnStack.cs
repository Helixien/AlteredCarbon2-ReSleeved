    ...
    public class Command_ActionOnStack : Command_ActionOnThing
    {
        ...
        public override IEnumerable<FloatMenuOption> FloatMenuOptions
        {
            get
            {
                ...
                {
                    if (neuralStack.Needlecasting)
                    {
                        yield return new FloatMenuOption(neuralStack.NeuralData.PawnNameColored + ": " + "AC.CannotModifyWhileNeedlecasting".Translate(info.recipe.label.ToLower()), null, iconThing: neuralStack, iconColor: Color.white);
                    }
                    else
                    {
                ...
                {
                    if (pawn.HasNeuralStack(out var hediff_NeuralStack) && hediff_NeuralStack.Needlecasting)
                    {
                        yield return new FloatMenuOption(pawn.NameShortColored + ": " + "AC.CannotModifyWhileNeedlecasting".Translate(info.recipe.label.ToLower()), null, iconThing: pawn, iconColor: Color.white);
                    }
                    else
                    {
